...
const BlogOutlineGenerator: React.FC = () => {
    ...
    const handleGenerate = useCallback(async () => {
        if (!topic) {
            setError('Por favor, insira um tópico para o esboço.');
            return;
        }
        setIsLoading(true);
        setError(null);
        setOutline([]);
        try {
            const result = await geminiService.generateBlogOutline(topic);
            setOutline(result);
        } catch (e) {
    ...
    return (
        <div className="space-y-6 max-w-4xl mx-auto">
            <div className="bg-slate-800 p-6 rounded-xl shadow-lg">
                <h2 className="text-2xl font-bold mb-4 text-white">Gerador de Esboço de Post de Blog</h2>
                <p className="text-slate-400 mb-4">Porque escrever um post de blog é fácil quando você tem um plano! Insira um tópico ou título.</p>
                <textarea
                    value={topic}
                    onChange={(e) => setTopic(e.target.value)}
                    placeholder="Ex: Como iniciar um podcast de sucesso"
            ...
            {(isLoading || outline.length > 0) && (
                <div className="bg-slate-800 p-6 rounded-xl shadow-lg">
                    {isLoading ? (
                        ...
                        <div>
                            <h3 className="text-xl font-bold text-purple-300 mb-2">Esboço Sugerido</h3>
