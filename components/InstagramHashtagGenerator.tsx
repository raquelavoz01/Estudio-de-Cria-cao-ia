...
const InstagramHashtagGenerator: React.FC = () => {
    ...
    const handleGenerate = useCallback(async () => {
        if (!topic) {
            setError('Por favor, descreva o tópico da sua postagem.');
            return;
        }
        setIsLoading(true);
        setError(null);
        setHashtags([]);
        try {
            const result = await geminiService.generateInstagramHashtags(topic);
            setHashtags(result);
        } catch (e) {
    ...
    return (
        <div className="space-y-6 max-w-4xl mx-auto">
            <div className="bg-slate-800 p-6 rounded-xl shadow-lg">
                <h2 className="text-2xl font-bold mb-4 text-white">Gerador de Hashtags do Instagram</h2>
                <p className="text-slate-400 mb-4">Crie hashtags para seu próximo Instagram Reels ou postagem e alcance mais pessoas!</p>
                <textarea
                    value={topic}
                    onChange={(e) => setTopic(e.target.value)}
                    placeholder="Ex: uma receita de bolo de chocolate caseiro"
